...
function Datepair(container, options) {
	...
	this._defaults =	{
		...
		parseDate: function(input){
			return input.value && jq(input).bdatepicker('getDate');
			//return input.value && jq(input).datepicker('d');

            //(t.datepicker._get(s,"dateFormat"),
            //s.input?s.input.val():null,t.datepicker._getFormatConfig(s))
            //return input.value && jq(input).datepicker.parseDate(jq(input).datepicker._get(input,"dateFormat"),input.value,jq(input).datepicker._getFormatConfig(input));
		},
		updateDate: function(input, dateObj){
			jq(input).bdatepicker('update', dateObj);
		}
	};
